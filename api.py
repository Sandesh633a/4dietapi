...
# --- API Key Configuration ---
API_KEYS_STR = os.getenv("VALID_API_KEYS", "dev-key-1,prod-key-xyz")
VALID_API_KEYS = {key.strip() for key in API_KEYS_STR.split(',') if key.strip()}
...
def load_and_clean_food_data(file_path):
    ...
    df = pd.read_csv(file_path, on_bad_lines='skip', skipinitialspace=True)
    df.columns = [col.strip() for col in df.columns]
    numeric_cols = ["Serving_Size_g", "Calories_kcal", "Protein_g", "Carbohydrates_g", "Fat_g", "Fiber_g"]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
    for col in ["Dish_Name", "Category", "Ayurvedic_Rasa", "Ayurvedic_Virya", "Ayurvedic_Guna", "Notes"]:
        if col in df.columns:
            df[col] = df[col].astype(str).str.strip()
    return df
...
def calculate_calorie_target(patient):
    try:
        weight_kg = float(patient["vitals"]["weight_kg"])
        height_cm = float(patient["vitals"]["height_cm"])
        # Accommodate both string and number for age
        age = int(patient["personalInfo"]["age"])
        gender = safe_lower(patient["personalInfo"]["gender"])
        activity_level = safe_lower(patient["lifestyle"]["physicalActivity"])
        activity_factors = {"low": 1.2, "moderate": 1.55, "high": 1.725}
...
def apply_filters_to_food_df(df, patient):
    ...
    # Standard Dosha and Condition filters
    if "pitta" in dosha_imbalance:
        filtered_df = filtered_df[~filtered_df["Ayurvedic_Virya"].apply(lambda x: "hot" in safe_lower(x))]
        for rasa in ["pungent", "sour", "salty"]:
            filtered_df = filtered_df[~filtered_df["Ayurvedic_Rasa"].apply(lambda x: rasa in safe_lower(x))]
    ...
    ## ENHANCED: Check for both diabetes and prediabetes
    if any(c in ["diabetes", "prediabetes"] for c in conditions):
        filtered_df = filtered_df[~filtered_df["Ayurvedic_Guna"].apply(lambda x: "difficult" in safe_lower(x))]
        filtered_df = filtered_df[~filtered_df["Category"].apply(lambda x: "desserts" in safe_lower(x))]
        
    ## ENHANCED: Filter based on Agni (digestive fire) status
    agni_status = safe_lower(patient.get("ayurvedaProfile", {}).get("agni", "sama"))
    if agni_status in ["manda", "visham"]: # Slow or irregular digestion
        filtered_df = filtered_df[filtered_df["Ayurvedic_Guna"].apply(lambda x: "easy" in safe_lower(x))]
    ...
    ## CORRECTED & ENHANCED: Filter based on patient's reported foods to avoid
    foods_to_avoid = patient.get("dietaryPreferences", {}).get("avoidFoods", [])
    if foods_to_avoid:
        avoid_keywords = []
        for phrase in foods_to_avoid:
            words = safe_lower(phrase).split()
            if "spicy" in words: avoid_keywords.append("spicy")
            if "fried" in words: avoid_keywords.append("fried")
            if "fermented" in words: avoid_keywords.append("fermented")
        
        if avoid_keywords:
            avoid_pattern = '|'.join(list(set(avoid_keywords))) # e.g. 'spicy|fried|fermented'
            filtered_df = filtered_df[~filtered_df["Dish_Name"].str.contains(avoid_pattern, case=False, na=False)]
            filtered_df = filtered_df[~filtered_df["Notes"].str.contains(avoid_pattern, case=False, na=False)]
...
def adjust_servings_for_calorie_target(day_plan, calorie_target):
    initial_calories = sum(item["Calories_kcal"] for meal in day_plan["meals"].values() for item in meal["items"])
    if initial_calories == 0:
        return day_plan, 0
    adjustment_factor = calorie_target / initial_calories
    adjustment_factor = max(0.7, min(1.5, adjustment_factor))
    adjusted_total_calories = 0
    for meal_name, meal_content in day_plan["meals"].items():
        for item in meal_content["items"]:
            original_serving = item["Serving_Size_g"]
            original_calories = item["Calories_kcal"]
            item["Adjusted_Serving_Size_g"] = int(round((original_serving * adjustment_factor) / 5) * 5)
            item["Adjusted_Calories_kcal"] = int(round(original_calories * adjustment_factor))
            item["Protein_g"] *= adjustment_factor
            item["Carbohydrates_g"] *= adjustment_factor
            item["Fat_g"] *= adjustment_factor
            item["Fiber_g"] *= adjustment_factor
            adjusted_total_calories += item["Adjusted_Calories_kcal"]
    return day_plan, adjusted_total_calories
        ...
        for meal_name, slots in meal_templates.items():
            ...
            for slot_categories in slots:
                ...
                if not available_candidates.empty:
                    ...
                    dish_id = int(chosen_dish["Dish_ID"])
                    dish_info = {
                        "Dish_ID": dish_id, "Dish_Name": chosen_dish["Dish_Name"], "Category": chosen_dish["Category"],
                        "Serving_Size_g": int(chosen_dish["Serving_Size_g"]), "Calories_kcal": int(chosen_dish["Calories_kcal"]),
                        "Protein_g": float(chosen_dish["Protein_g"]), "Carbohydrates_g": float(chosen_dish["Carbohydrates_g"]),
                        "Fat_g": float(chosen_dish["Fat_g"]), "Fiber_g": float(chosen_dish["Fiber_g"]),
                        "Ayurvedic_Rasa": chosen_dish["Ayurvedic_Rasa"], "Ayurvedic_Virya": chosen_dish["Ayurvedic_Virya"],
                        "Ayurvedic_Guna": chosen_dish["Ayurvedic_Guna"], "Notes": chosen_dish["Notes"].strip()
                    }
                    meal_items.append(dish_info)
                    all_day_dishes.append(dish_info)
                    used_dishes_count[dish_id] = used_dishes_count.get(dish_id, 0) + 1
            day_plan["meals"][meal_name] = {"items": meal_items}

        day_plan, adjusted_calories = adjust_servings_for_calorie_target(day_plan, calorie_target)
        day_plan["daily_total_calories_kcal"] = adjusted_calories
        rasa_counts = defaultdict(int)
        virya_counts = defaultdict(int)
        for dish in all_day_dishes:
            virya = dish["Ayurvedic_Virya"].strip().capitalize()
            if virya: virya_counts[virya] += 1
            rasas = [r.strip().capitalize() for r in dish["Ayurvedic_Rasa"].split(',')]
            for rasa in rasas:
                if rasa: rasa_counts[rasa] += 1
        day_plan["daily_ayurvedic_summary"] = {
            "rasa_distribution": dict(rasa_counts), "virya_balance": dict(virya_counts)
        }
        plan["weekly_plan"].append(day_plan)
    return plan
...
try:
    ...
    print("INFO: Food database loaded successfully.")
    if not VALID_API_KEYS:
        print("CRITICAL_ERROR: No VALID_API_KEYS found. Set the env var to run securely.")
        sys.exit(1)
    print("INFO: API keys loaded successfully.")
except FileNotFoundError as e:
    print(f"CRITICAL_ERROR: {e}")
...
def create_diet_plan():
    patient_data = request.get_json()
    if not patient_data:
        return jsonify({"error": "Invalid request. Please provide patient data."}), 400
    try:
        suitable_food_df = apply_filters_to_food_df(food_df, patient_data)
        diet_plan = generate_7_day_plan(suitable_food_df, patient_data)
        return jsonify(diet_plan), 200
